...
const Slug = ({addToCart}) => {
  ...
  const handleCheckPin = async () =>{
    let pinFetch =await fetch("http://localhost:3000/api/pincode")
    let pinJson = await pinFetch.json()
    // pinJson = parseInt(pinJson)
  ...
  <div className="container px-5 md:py-24 mx-auto">
    <div className="lg:w-4/5 mx-auto flex flex-wrap justify-center">
        <a className="flex relative h-80 md:mt-10 mx-auto rounded overflow-hidden justify-center">
          <img alt="ecommerce" className=" m-auto md:m-0 h-[38vh] block" src="https://m.media-amazon.com/images/I/81OVmTN9voL._AC_UL1500_.jpg"/>
        </a>
      <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
        ...
        <div className="flex">
          ...
          <button className="flex ml-4 text-white bg-pink-500 border-0 py-2  px-2 md:px-6 focus:outline-none hover:bg-pink-600 rounded text-sm">Buy now</button>
          <button onClick={()=> {addToCart(slug, 1, 499, 'Wear the Code(XL, Red','XL', 'Red' )}} className="flex ml-4 text-white bg-pink-500 border-0 py-2 px-2 md:px-6 focus:outline-none hover:bg-pink-600 rounded text-sm">Add to Cart</button>
